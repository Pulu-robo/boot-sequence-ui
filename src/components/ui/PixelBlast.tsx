...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      console.log('PixelBlast canvas added - listening for window clicks');
      ...
      const onPointerDown = (e: PointerEvent) => {
        ...
        if (target) {
          ...
          if (isInteractive) {
            console.log('Click on interactive element, skipping ripple');
            return;
          }
        }
        
        console.log('PixelBlast click detected at:', e.clientX, e.clientY);
        const { fx, fy } = mapToPixels(e);
        const ix = threeRef.current?.clickIx ?? 0;
        uniforms.uClickPos.value[ix].set(fx, fy);
        uniforms.uClickTimes.value[ix] = uniforms.uTime.value;
        if (threeRef.current) threeRef.current.clickIx = (ix + 1) % MAX_CLICKS;
        console.log('Ripple created at index:', ix, 'position:', fx, fy);
      };
      const onPointerMove = (e: PointerEvent) => {
        ...
        if (threeRef.current) {
          ...
          const lastMoveTime = (threeRef.current as any).lastMoveTime || 0;
          if (now - lastMoveTime < 16) return; // ~60fps throttle
          (threeRef.current as any).lastMoveTime = now;
        }
        const { fx, fy, w, h } = mapToPixels(e);
        touch.addTouch({ x: fx / w, y: fy / h });
      };
      
      // Store event handlers in threeRef for cleanup
      if (threeRef.current) {
        threeRef.current.eventHandlers = {
          pointerDown: onPointerDown,
          pointerMove: onPointerMove
        };
      }
      
      // Listen on window to capture all clicks, then filter out interactive elements
    ...
    return () => {
      ...
      if (t.eventHandlers) {
        window.removeEventListener('pointerdown', t.eventHandlers.pointerDown);
        window.removeEventListener('pointermove', t.eventHandlers.pointerMove);
      }
  ...
  return (
    <div
      ...
      onClick={(e) => {
        console.log('Container clicked, propagating to canvas');
      }}
    />
  );
};
